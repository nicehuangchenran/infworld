...
冻结 TemporaLatentEncode 减小随着 chunk 增加激活值占用显存的问题
"""
...
def torch_gc():
    ...
    torch.cuda.ipc_collect() # 清理跨进程 IPC GPU 资源
...
NUM_CHUNKS = 10  # Number of video chunks to generate
...
def mem_record(tag="", optimizer=None, model=None, reset_peak=False):
    """打印当前显存占用，并按 模型 / 梯度 / 优化器 / 激活值 拆分。

    激活值无法直接查询，这里用差值估计：
        activations ≈ allocated - params - grads - optimizer_states
    其中 allocated 是 PyTorch 当前真正持有的显存（不含 caching allocator 里
    已释放但未归还的部分，那部分体现在 reserved 里）。

    reset_peak=True 会在打印后清零 max_memory_allocated，便于测量“下一段代码”
    （如一次 sample / 一次 train step）的峰值显存。
    """
...
def main():
    ...
    # 基线：模型加载 + 冻结完成后，只有模型权重常驻显存（此时无梯度/优化器/激活）
    mem_record(tag="after_model_load", model=dit, reset_peak=True)
    ...
    for task_idx, (prompt, image_path, action_path) in enumerate(target_prompts):
        ...
        for chunk_idx in range(NUM_CHUNKS):
            ...
            mem_record(tag=f"task{task_idx}_chunk{chunk_idx}_before_sample", model=dit, reset_peak=True)
            with torch.no_grad():
                ...
                # sample 阶段峰值（推理激活值随 chunk/历史变长而增长，是 OOM 关注点）
                mem_record(tag=f"task{task_idx}_chunk{chunk_idx}_after_sample", model=dit, reset_peak=True)
            ...
            if ENABLE_ONLINE_TRAINING and chunk_idx < NUM_CHUNKS - 1:
                ...
                for step in range(N_TRAIN_STEPS):
                    ...
                    # 第一步后：梯度 + 优化器状态(step1 后 AdamW 已创建 m/v) + 训练激活值全部就位
                    if step == 0:
                        mem_record(tag=f"task{task_idx}_chunk{chunk_idx}_train_step1",
                                   optimizer=optimizer, model=dit, reset_peak=True)
                # 训练结束（optimizer.step 后梯度仍在、激活已释放），观察训练段峰值
                mem_record(tag=f"task{task_idx}_chunk{chunk_idx}_after_train",
                           optimizer=optimizer, model=dit)
                del x_start, train_kwargs, optimizer
                torch_gc()
                # 清理后：梯度/优化器/激活应被回收，回到接近基线
                mem_record(tag=f"task{task_idx}_chunk{chunk_idx}_after_train_gc", model=dit)
